...
$query = "select * from out_bill_xml where subdivision_id='". $s ."' and mydate='". strtotime($b) ."' and down='1' order by id limit ". $xf .",".$xt ;
$q = mysql_query($query);
if(mysql_num_rows($q)>0){
	while($qd = mysql_fetch_object($q)){
		if(strtotime($qd->bill_datetime) <= strtotime($xd)){
			$data_arr[]=$qd;
		}
	}
}
...
if($pd !=""){
	$query = "select * from out_bill_xml where subdivision_id='". $s ."' and mydate='". strtotime($b) ."' and down='1' order by id limit 0,".$pt;
	$q = mysql_query($query);
	if(mysql_num_rows($q)>0){
		while($qd = mysql_fetch_object($q)){
			if((strtotime($pd) < strtotime($qd->bill_datetime))&& (strtotime($qd->bill_datetime) <= strtotime($xd))){
				$data_arr[]=$qd;
			}
		}
	}
