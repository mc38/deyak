...
function add_con(doc,mid){
	...
	if(d !="" ){
		...
		if(conf){
			...
			$.ajax({
				type:"POST",
				url:llink + "server/add_con.php",
				data:{c:c,d:dd},
				success:function(res){
					//alert(res);
					if(res == c){
						show_msg(doc,msgid,"#00500E","Add Completed");
						list_show(ufile_name);
					}
					else if(res == "0"){
						...
						reset_upload_file();
						show_msg(doc,msgid,"#98261A","Importation file not found");
					}
					else if(res == "2"){
						var msg ="May be some errors are there!";
						show_msg(doc,msgid,"#98261A",msg);
					}
					else{
...
function delete_con(doc){
	...
	if(d !="" ){
		...
		document.getElementById(msgid).innerHTML = "Please Wait";
		var c = getcode(10);
		var conf = confirm("Do you really want to delete this?");
		if(conf){
			...
			$.ajax({
				type:"POST",
				url:llink + "server/del_con.php",
				data:{c:c,d:dd},
				success:function(res){
					//alert(res);
					if(res == c){
						show_msg(doc,msgid,"#00500E","Del Completed");
						list_show(ufile_name);
					}
					else if(res == "0"){
						show_msg(doc,msgid,"#98261A","Authentication problem");
