...
if($c !=""){
	$where .= " and consumer_id like '%". $c ."'";
}

$twhere ="";
if($t !=""){
	$twhere = " and tariff_id like '". $t ."%'";
}


$q = mysql_query("select * from out_reading_xml where subdivision_id='". $s ."' and mydate='". strtotime($b) ."' and down='1'". $where ." order by id");
if(mysql_num_rows($q)>0){
	...
	while($datad = mysql_fetch_object($q)){
		...
		$cq = mysql_query("select id from p_consumerdata where subdiv_id='". $s ."' and mydate='". strtotime($b) ."' and id='".$datad->link."'".$twhere);
	...
	$pos = $j-1;
	if(($pos % $buff < $buff-1) && ($pos % $buff >=0) && $pos>0){
		$xml->formatOutput = true;
		$j = $j-1;
		$fno = (int)($j/$buff)+1;
		$ffrm = (($fno-1) * $buff) +1;
