...
if(authenticate()){
	if(
		...
		$trbq = mysql_query("select * from trash_p_billdata where id='". $id ."'");
		if(mysql_num_rows($trbq) >0){
			$trbd = mysql_fetch_object($trbq);
			$link = $trbd->link;
			
			$cq = mysql_query("select * from p_consumerdata where id='".$link."'");
			$cd = mysql_fetch_object($cq);
			
			$tbq = mysql_query("select * from p_billdata where link='".$link."' and status=''");
			if(mysql_num_rows($tbq) >0){
				...
				if(mysql_num_rows($b_ex_q)<1){
					...
					$bd->status				= $trbd->status;
					$bd->postmeter_read		= $postmeter_read;
					$bd->meterpic			= $trbd->meterpic;
					$bd->reading_date		= substr($trbd->reading_date,0,10);
					...
					$bdcol[5] = "fmeterno";					$bdval[5] =$bd->$bdcol[5];
