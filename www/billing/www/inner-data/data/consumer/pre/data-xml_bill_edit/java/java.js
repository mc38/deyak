...
/////////////
function show_pdate(doc){
	var d = doc.value;
	clear_field();
	if(d>0){
		$("#process_date").css({"display":""});
	}else{
		$("#process_date").css({"display":"none"});
	}
}
...
function list_search(doc){
	...
	var dty = document.getElementById("dty").value;
	...
	if(str !="" && smon !="" && syar !="" && dty !=""){
		var sdate = "01-"+ smon +"-"+ syar;
		
		var data = [str.toLowerCase(),sdate,dty,aid,bkno,pdt,cid];
