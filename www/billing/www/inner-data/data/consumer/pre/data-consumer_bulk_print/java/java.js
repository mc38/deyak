			...
			success:function(res){
				...
				if(res == "0"){
					...
					if(rd_arr[0] == c){
						...
						var i; var ag_html='<option value="">Select Agent</option><option value="0">Office Process</option>';
...
function list_search(doc){
	...
	var rdate = document.getElementById("reading_date").value;
	var bkno = document.getElementById("book_no").value;
	var cid = document.getElementById("conid").value;
	
	
	if(str !="" && smon !="" && syar !="" && (aid !="" || cid !="")){
		var sdate = "01-"+ smon +"-"+ syar;
		
		var data = [str.toLowerCase(),sdate,aid,rdate,bkno,cid];
