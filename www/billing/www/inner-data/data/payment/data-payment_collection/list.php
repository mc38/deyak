...
if(authenticate()){
	...
	if(mysql_num_rows($subdq)==1){
			...
			$where = "";
			if($cd !=""){
				$where .= " and datetime like '".date('d-m-Y',strtotime($cd))."%'";
			}
			if($ai !=""){
				$where .= " and aid='".$ai."'";
			}
			...
			if(mysql_num_rows($q) >0){
				...
				echo '	<tr><th>Slno</th>	<th>Consumer ID</th>	<th>Name</th>	<th>Agent</th>	<th>Collection Date</th>	<th>Collection Amount</th>';
				if($lt <1){
					echo '	<th>Commission</th>		<th>Total</th</tr>';
				}
				...
				while($d = mysql_fetch_object($q)){
					...
					$cd = mysql_fetch_object($cq);
						...
						echo '<td align="center">'. substr($d->datetime,0,10) .'</td>';
						echo '<td align="right">Rs '. number_format($d->amount,2) .'/-</td>';
						$am_t = $am_t + $d->amount;
